...
import type { ActorInitTypeahead } from './ActorInitTypeahead';
...
export function newEngineDynamicArged(options: IQueryOptions, moduleRootPath: string,
  defaultConfigPath: string): Promise<ActorInitTypeahead> {
  ...
  return instantiateComponent(configResourceUrl, runnerInstanceUri, options)
    .then((runner: Runner) => <ActorInitTypeahead> runner.collectActors({ engine: instanceUri }).engine);
}
